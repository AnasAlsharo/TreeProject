...
if ~checkCollinearity(X)
    [K, A] = convhull(X(:,1), X(:,2));
else
    disp('Points are collinear. Convex hull computation skipped.');
end
...
function isCollinear = checkCollinearity(points)
    ...
    isCollinear = true;
    for i = 1:n-2
        % Calculate the area of the triangle formed by points i, i+1, and i+2
        x1 = points(i, 1);
        y1 = points(i, 2);
        x2 = points(i+1, 1);
        y2 = points(i+1, 2);
        ...
        if area > 1e-5 % Adjust this tolerance as necessary
            isCollinear = false;
            break;
        end
    end
